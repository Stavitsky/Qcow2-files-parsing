...
def parse_dirs(src, dirs=0, files=0, qfi_files=0, files_data=None):
    """ returns array of wcow files-data and counts total files, dirs and qcow files
    ...
    for f in os.listdir(src): #for each file in current folder
        file_wp = os.path.join(src, f) #full path to file (with folders)
